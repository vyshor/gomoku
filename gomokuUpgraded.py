...
#                     st += move + "|"
#                 print(st)
def render_board(move=(-1, -1)):
    global Width, x, y, game, lastmovebox
    boxwidth = Width / boardsize
    if move == (-1, -1):  # if board is not rendered yet
        for i in range(boardsize):
            for j in range(boardsize):
                board.create_rectangle(i * boxwidth, j * boxwidth, (i + 1) * boxwidth, (j + 1) * boxwidth, fill="white",
                                       width=1)

        i, j = 0, 0
        lastmovebox = board.create_rectangle(i * boxwidth + .1 * boxwidth, j * boxwidth + .1 * boxwidth,
                                             (i + 1) * boxwidth - .1 * boxwidth, (j + 1) * boxwidth - .1 * boxwidth,
                                             fill="green",
                                             width=0, tag="lastmovebox")

    else:  # if board is rendered around, but another player makes a move
        i, j = move
        if game[i][j] == 1:
            board.create_oval(i * boxwidth + .2 * boxwidth, j * boxwidth + .2 * boxwidth,
                              (i + 1) * boxwidth - .2 * boxwidth, (j + 1) * boxwidth - .2 * boxwidth, width=2)

        elif game[i][j] == 2:
            board.create_line(i * boxwidth + .2 * boxwidth, j * boxwidth + .2 * boxwidth,
                              (i + 1) * boxwidth - .2 * boxwidth, (j + 1) * boxwidth - .2 * boxwidth, width=4)
            board.create_line(i * boxwidth + .2 * boxwidth, j * boxwidth + .8 * boxwidth,
                              (i + 1) * boxwidth - .2 * boxwidth, (j + 1) * boxwidth - .8 * boxwidth, width=4)

        board.coords(lastmovebox, i * boxwidth + .2 * boxwidth, j * boxwidth + .2 * boxwidth,
                     (i + 1) * boxwidth - .2 * boxwidth, (j + 1) * boxwidth - .2 * boxwidth)
